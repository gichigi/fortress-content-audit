...
export async function crawlSite(startUrl, limit = 20) {
  ...
  try {
    ...
    // pollInterval: 2 seconds (fast polling to keep under 10s total)
    // limit: cap at 3 pages for MVP to save credits
 ...
 */
export async function mapSite(startUrl, limit = 50) {
  const apiKey = process.env.FIRECRAWL_API_KEY
  if (!apiKey) {
    return { success: false, pages: [], error: 'Missing FIRECRAWL_API_KEY' }
  }
